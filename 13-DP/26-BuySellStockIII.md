...
class Solution {
private:
    int rec(int index, int buy, int cap, vector<int> &arr){
      int n = arr.size();
      if(index == n || cap == 0) return 0;
      int profit = 0;
      if(buy){
        profit = max(-arr[index] + rec(index + 1, 0, cap, arr),
                      0 + rec(index + 1, 1, cap, arr));
      } else{
        profit = max( arr[index] + rec(index + 1, 1, cap-1, arr),
                      0 + rec(index + 1, 0, cap, arr));
      }
      return profit;
    }
public:
    int maxProfit(vector<int>& arr) {
        return rec(0, 1, 2, arr);
    }
};
